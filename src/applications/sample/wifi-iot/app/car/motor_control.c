...
void LeftWheelForward(int duty)
{
    IoTPwmStart(IOT_PWM_PORT_PWM2, duty, IOT_FREQ);
}

void LeftWheelBackword(int duty)
{
    IoTPwmStart(IOT_PWM_PORT_PWM3, duty, IOT_FREQ);
}
...
void RightWheelForward(int duty)
{
    IoTPwmStart(IOT_PWM_PORT_PWM1, duty, IOT_FREQ);
}

void RightWheelBackword(int duty)
{
    IoTPwmStart(IOT_PWM_PORT_PWM0, duty, IOT_FREQ);
}
...
void car_backward(uint32_t left_pwm_value, uint32_t right_pwm_value){
    car_stop();
    LeftWheelBackword(left_pwm_value);
    RightWheelBackword(right_pwm_value);
}
void car_forward(uint32_t left_pwm_value, uint32_t right_pwm_value){
    car_stop();
    LeftWheelForward(left_pwm_value);
    RightWheelForward(right_pwm_value);
}
void car_left(uint32_t right_pwm_value){
    car_stop();
    LeftWheelForward(right_pwm_value);
}
void car_right(uint32_t left_pwm_value){
    car_stop();
    RightWheelForward(left_pwm_value);
}
void car_stop(void){
    IoTPwmStop(IOT_PWM_PORT_PWM0);
    IoTPwmStop(IOT_PWM_PORT_PWM1);
